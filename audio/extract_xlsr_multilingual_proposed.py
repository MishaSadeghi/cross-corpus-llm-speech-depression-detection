"""
Extract facebook/wav2vec2-large-xlsr-53 (multilingual) features from the proposed dataset.

This script is used for fixed-split evaluations and cross-corpus experiments
(Tables 4 & 5 in the paper), where the same language-agnostic model is applied
to both the proposed dataset and E-DAIC to ensure comparability.
...
Output saved to:
  FEATURES_BASE/proposed_dataset_xlsr53_multilingual_{mode}/proposed_dataset_xlsr53_multilingual_{mode}.csv
"""
...
# Local model directory (optional; avoids re-downloading from HuggingFace Hub)
...
def main():
    ...
    out_dir = FEATURES_BASE / f"proposed_dataset_xlsr53_multilingual_{mode}"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_csv = out_dir / f"proposed_dataset_xlsr53_multilingual_{mode}.csv"
    ...
    logging.info(f"\n{'='*60}")
    logging.info(f"EXTRACTION COMPLETE — Proposed Dataset XLSR-53 Multilingual ({mode})")
