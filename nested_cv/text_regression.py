"""
MAXIMAL Text-Based PHQ-9 Regression with CV-Aware DepRoBERTa

This script predicts PHQ-9 scores using regression models with fold-specific
DepRoBERTa features to prevent data leakage.

Workflow:
- Fine-tunes DepRoBERTa for each CV fold on 3-class PHQ-9 (training data only)
- Extracts 3 probability features using fold-specific models
- Merges with existing 11 question features (already leakage-free)
- Uses regression models to predict PHQ-9 scores (0-27)
...
# PHQ-9 scores used for BOTH:
# 1. DepRoBERTa fine-tuning (mapped to 3-class categorical)
# 2. Final regression target (continuous PHQ-9 scores)
PHQ_LABELS_CSV = Path("CONFIGURE_ME/labels/regress_labels.csv")
OUTPUT_DIR = Path("CONFIGURE_ME/storage/results/text_nested_cv_regression")
...
def run_experiment(llama_model: str, quality_name: str, quality_filter):
    ...
    # Load PHQ-9 labels for regression task
    labels_df = pd.read_csv(PHQ_LABELS_CSV)  # Contains 'patient_id' and 'PHQ9-Score'
