"""
MAXIMAL Text-Based PHQ-8 Regression with CV-Aware DepRoBERTa

This script predicts PHQ-8 scores using regression models with fold-specific
DepRoBERTa features to prevent data leakage.

Workflow:
- Fine-tunes DepRoBERTa for each CV fold on 3-class PHQ-8 (training data only)
- Extracts 3 probability features using fold-specific models
- Merges with existing 11 question features (already leakage-free)
- Uses regression models to predict PHQ-8 scores (0-24)
- Auto-generates summary with mean±std metrics

Total: 3 Llama models × 1 filter × 12 regressors × 5 folds = 180 experiments
"""
import json
import logging
import random
...
# PHQ-8 scores used for BOTH:
# 1. DepRoBERTa fine-tuning (mapped to 3-class categorical)
# 2. Final regression target (continuous PHQ-8 scores)
PHQ_LABELS_CSV = Path("CONFIGURE_ME/labels/regress_labels_phq8.csv")
OUTPUT_DIR = Path("CONFIGURE_ME/storage/results/text_nested_cv_regression_phq8")
...
MODEL_SAVE_DIR = WOODY_BASE / "deproberta_cv_models_phq8"
MODEL_SAVE_DIR.mkdir(parents=True, exist_ok=True)

# Feature storage: Extracted probability features from DepRoBERTa
FEATURES_SAVE_DIR = WOODY_BASE / "deproberta_extracted_features_phq8"
...
# Regressor hyperparameter grids
GRIDS = {
    "Ridge": {
...
def get_exclude_ids(quality_filter):
    ...
    if quality_filter == "inverse_good":
        good_ids = load_quality_ids(BASE_DIR / "Other/nisqa_good_quality_ids.txt")
        very_good_ids = load_quality_ids(BASE_DIR / "Other/nisqa_very_good_quality_ids.txt")
...
def fine_tune_deproberta_for_fold(
    ...
    # Load PHQ-8 labels for fine-tuning DepRoBERTa (3-class categorical)
...
def run_experiment(llama_model: str, quality_name: str, quality_filter):
    ...
    # Load PHQ-8 labels for regression task
    labels_df = pd.read_csv(PHQ_LABELS_CSV)  # Contains 'patient_id' and 'PHQ8-Score'
    ...
    print(f"PHQ-8 range: {y.min():.1f} - {y.max():.1f}, mean: {y.mean():.1f}±{y.std():.1f}")
    ...
    for fold, (train_idx, test_idx) in enumerate(outer_cv.split(all_patient_ids, y)):
        ...
        # STEP 4: Run regression with all models
        for model_name in GRIDS.keys():
            ...
            try:
                ...
                # Inner CV for hyperparameter tuning (KFold for regression)
                ...
                logging.error(f"Regression failed for {model_name}: {e}")
                continue
    
    return results

def summarize_results(df: pd.DataFrame, output_dir: Path):
    """Generate mean±std summary across CV folds."""
    grouped = df.groupby(['llama_model', 'quality_filter', 'regressor'])
    
    summary_rows = []
    for (model, quality, regressor), group in grouped:
        summary_rows.append({
            'llama_model': model,
    ...
    summary_df = pd.DataFrame(summary_rows).sort_values('r2_mean', ascending=False)
    ...
    for _,row in top10.iterrows():
        print(f"{row['regressor']:20s} ({row['llama_model']}): "
...
def main():
    print("="*70)
    print("MAXIMAL TEXT REGRESSION - CV-AWARE DEPROBERTA")
    print("="*70)
    
    set_seed(42)
    
    n_regressors = len(GRIDS)
    print(f"\nTesting {n_regressors} regressors")
    print(f"{len(LLM_SUMMARY_DIRS)} Llama models × {len(QUALITY_FILTERS)} Quality filters × {n_regressors} regressors × 5 folds")
    print(f"Total experiments: {len(LLM_SUMMARY_DIRS) * len(QUALITY_FILTERS) * n_regressors * 5}")
    ...
    if all_results:
        ...
        output_file = OUTPUT_DIR / "maximal_text_regression_cv_deproberta_results.csv"
        df.to_csv(output_file, index=False)
        
        print("\n" + "="*70)
        print("RESULTS SUMMARY")
        print("="*70)
        print(f"\n✓ Fold-level results saved to: {output_file}")
        
        # Auto-generate summary with mean±std
