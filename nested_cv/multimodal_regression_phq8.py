"""
MULTIMODAL PHQ-8 Regression with CV-Aware DepRoBERTa
...
# ---- v2: all four audio feature sets --------------------------------
AUDIO_SOURCES = {
...
# PHQ-8 scores (used for DepRoBERTa fine-tuning AND regression target)
PHQ_LABELS_CSV = Path("CONFIGURE_ME/labels/regress_labels_phq8.csv")

# Output directory — phq8 suffix, no overwrite of PHQ-9 results
OUTPUT_DIR = Path("CONFIGURE_ME/storage/results/multimodal_nested_cv_regression_phq8")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# CONFIGURE_ME: STORAGE_ROOT should be on a large filesystem
WOODY_BASE = Path("CONFIGURE_ME/storage")

# Fine-tuned DepRoBERTa models (namespaced under 'multimodal_regression_phq8')
MODEL_SAVE_DIR = WOODY_BASE / "deproberta_cv_models_multimodal_regression_phq8"
MODEL_SAVE_DIR.mkdir(parents=True, exist_ok=True)

# Extracted DepRoBERTa features (per fold)
FEATURES_SAVE_DIR = WOODY_BASE / "deproberta_extracted_features_multimodal_regression_phq8"
...
def fine_tune_deproberta_for_fold(
    ...
    labels_df['PHQ8-Score'] = pd.to_numeric(labels_df['PHQ8-Score'], errors='coerce')
    labels_df.dropna(subset=['PHQ8-Score'], inplace=True)
    labels_df['label'] = labels_df['PHQ8-Score'].apply(map_phq_to_label)
...
def run_experiment(llama_model: str) -> List[Dict]:
    ...
    For each of 5 outer folds × 4 audio sources:
      1. Fine-tune DepRoBERTa on TRAIN patients (3-class PHQ-8 categorical)
    ...
    for audio_source_name in AUDIO_SOURCES:
        ...
        print(f"PHQ-8 range: {y_all.min():.1f}–{y_all.max():.1f}, mean: {y_all.mean():.1f}±{y_all.std():.1f}")
...
def generate_shap_plots(df: pd.DataFrame) -> None:
    ...
    title = (
        "Multimodal PHQ-8 Regression — SHAP Feature Importance (Beeswarm)\n"
...
def main(args=None):
    print("=" * 70)
    print("MULTIMODAL PHQ-8 REGRESSION — CV-AWARE DEPROBERTA")
    print("=" * 70)
    print()
    ...
    print(f"Target:            PHQ-8 continuous score (0–27)")
    print()
    ...
    if all_results:
        ...
        if args is not None and args.llama_model:
            output_file = OUTPUT_DIR / f"multimodal_cv_deproberta_regression_results_v2_phq8_{args.llama_model}.csv"
        else:
            output_file = OUTPUT_DIR / "multimodal_cv_deproberta_regression_results_v2_phq8.csv"
